...
public class AuthorizationFilter implements ContainerRequestFilter {
				...
				try {
					if (this.dao.checkLogin(userPass[0], userPass[1])) {
						...
						if (isPeoplePath(requestContext) && !user.getRole().equals("ADMIN")) {
							...
							if (isPetPath(requestContext) && !user.getRole().equals("ADMIN")) {
								requestContext.abortWith(createResponse());
							}
							else{requestContext.setSecurityContext(new UserSecurityContext(user));
							}
						}
					} else {
						requestContext.abortWith(createResponse());
	...
	private static boolean isPetPath(ContainerRequestContext context) {
		final List<PathSegment> pathSegments = context.getUriInfo().getPathSegments();
		return !pathSegments.isEmpty() && pathSegments.get(0).getPath().equals("pet");
	}
